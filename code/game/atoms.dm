/atom
	layer = 2
	var/level = 2//Used by pipes and wires to hide under the floor
	var/flags = FPRINT
	...
	if(istype(hit_atom,/mob/living))
		...
		if(T.density)
			...
			if(istype(src,/mob/living))
				...
				M.deal_damage(20, BRUTE, IMPACT, "chest")//Now properly checks armor
...
/atom/proc/bullet_act(var/obj/item/projectile/Bullet)
	Bullet.on_hit(src)
	return 1
...
*/
/atom/proc/Beam(atom/BeamTarget,icon_state="b_beam",icon='icons/effects/beam.dmi',time=50, maxdistance=10, delay=3)
	...
	//continue to draw.
		...
		for(N,N<length,N+=32)
			...
			X.pixel_x=Pixel_x
			X.pixel_y=Pixel_y
		sleep(delay)	//Changing this to a lower value will cause the beam to follow more smoothly with movement, but it will also be more laggy.
...
//	world << "atom.DblClick() on [src] by [usr] : src.type is [src.type]"
	...
	if(usr.stat)
		return
	if((usr.paralysis || usr.weakened) && !istype(usr, /mob/living/silicon/ai))
		return
...
//	world << "according to dblclick(), t5 is [t5]"
	...
	if (((t5 || (W && (W.flags & USEDELAY))) && !( istype(src, /obj/screen) )))
		...
		if(usr.next_move >= world.time)
			return 0	//Waiting for the delay
		...
		if ((src.loc && (get_dist(src, usr) < 2 || src.loc == usr.loc)))
			...
			var/ok = 1//>0 means we can reach it
			if((direct - 1) & direct)
				// ------- CLICKED OBJECT IS LOCATED IN A DIAGONAL POSITION FROM THE PERSON -------
				var/obj/item/weapon/dummy/D = new /obj/item/weapon/dummy(usr.loc)//Creating a dummy to detect if we can reach the thing we clicked on
				var/turf/Step_1
				var/turf/Step_2
				switch(direct)
					if(5.0)
				...
				if(!Step_1 || !Step_2)//San check
					return 0
					// ------- BOTH CARDINAL DIRECTIONS OF THE DIAGONAL EXIST IN THE GAME WORLD -------

				if(!step_to(D, Step_1))//Dummy attempts to move in the first direction
					D.loc = usr.loc//If it failed then we reset the loc and try the other way
					if(!step_to(D, Step_2))
						ok = 0//Both attempts have failed, we cant reach it
				D.loc = null//Need to remove the dummy now
				D = null
					/*
						More info:
							...
							These two tiles are Step_1 and Step_2. After this, a new dummy object is created on your location. It then tries to move to Step_1,
							CheckExit() and CanPass() are use to determine this. If it fails then the dummy object is moved back to your location and it tries to move to Step_2. Same checks are performed here.
							If at least one of the two checks succeeds, it means you can reach the item and ok is set to 1.
					*/

			if(!ok)//Can we reach the tile or not
				return 0

		if(usr.restrained() || (usr.lying && usr.buckled!=src))//If we are restrained then call the proper proc, afiak more or less none of these are used ever
			if (istype(usr, /mob/living/carbon/human))
				return src.hand_h(usr, usr.hand)
			else if (istype(usr, /mob/living/carbon/monkey))
				return src.hand_p(usr, usr.hand)
			else if (istype(usr, /mob/living/carbon/alien/humanoid))
				return src.hand_al(usr, usr.hand)
			else if (istype(usr, /mob/living/silicon/ai) || istype(usr, /mob/living/silicon/robot))
				return src.hand_a(usr, usr.hand)
			return 0//You have no restrained proc

		if(W)//You have an item, call attackby and after attack
			if(t5)//You are close enough to hit target with your item
				src.attackby(W, usr)
			if(W)//Always called, ranged weapons tend to use this all the time
				W.afterattack(src, usr, (t5 ? 1 : 0), params)
			return 1

		//No item, call user clicking on things proc
		if (istype(usr, /mob/living/carbon/human))
			return src.attack_hand(usr, usr.hand)
		else if (istype(usr, /mob/living/carbon/monkey))
			return src.attack_paw(usr, usr.hand)
		else if (istype(usr, /mob/living/carbon/alien/humanoid))
			if(usr.m_intent == "walk" && istype(usr, /mob/living/carbon/alien/humanoid/hunter))
				usr.m_intent = "run"
				usr.hud_used.move_intent.icon_state = "running"
				usr.update_icons()
			return src.attack_alien(usr, usr.hand)
		else if (istype(usr, /mob/living/carbon/alien/larva))
			return src.attack_larva(usr)
		else if (istype(usr, /mob/living/silicon/ai) || istype(usr, /mob/living/silicon/robot))
			return src.attack_ai(usr, usr.hand)
		else if(istype(usr, /mob/living/carbon/slime))
			return src.attack_slime(usr)
		else if(istype(usr, /mob/living/simple_animal))
			return src.attack_animal(usr)
		else
			return 0//Whatever you are has no standard click proc
		return 0//Failsafe if we somehow passed everything else
...
/*	else why the fuck is this here I ALREADY REMOVED YOU ONCE.  SHIT LIKE THIS DOES NOT GO IN A PLACE LIKE CLICK
...
/atom/proc/get_global_map_pos()
	if(!islist_legacy(global_map) || isemptylist(global_map)) return
	var/cur_x = null
	var/cur_y = null
	var/list/y_arr = null
